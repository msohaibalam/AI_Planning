...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # no steps required if already at goal state
        if self.goal_test(node.state):
            return 0
        # initialize count to 0
        count = 0
        # keep track of each looped over effect (of each action) to avoid over-counting
        effs_observed = []
        for a in self.actions_list:
            effs = a.effect_add
            for e in effs:
                # increment count if effect is in one of the goals
                if (e in self.goal) and (e not in effs_observed):
                    count += 1
                effs_observed.append(e)
        # # return infinite if problem impossible
        # if count == 0:
        #     return float("inf")
        return count
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), expr('At(C1, ATL)'), expr('In(C1, P1)'), expr('In(C1, P2)'), expr('In(C1, P3)'),
           expr('At(C2, SFO)'), expr('At(C2, ATL)'), expr('In(C2, P1)'), expr('In(C2, P2)'), expr('In(C3, P3)'),
           expr('At(C3, SFO)'), expr('At(C3, JFK)'), expr('In(C3, P1)'), expr('In(C3, P2)'), expr('In(C3, P3)'),
           expr('At(P1, JFK)'), expr('At(P1, ATL)'), expr('At(P2, SFO)'), expr('At(P2, ATL)'),
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), expr('At(C1, ATL)'), expr('At(C1, ORD)'),
           ...
           expr('In(C1, P1)'), expr('In(C1, P2)'), expr('In(C2, P1)'), expr('In(C2, P2)'),
           expr('In(C3, P1)'), expr('In(C3, P2)'), expr('In(C4, P1)'), expr('In(C4, P2)'),
