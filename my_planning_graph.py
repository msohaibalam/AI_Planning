...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # identify all the True and False literals in the current state level
        pos_literals = [literal.symbol for literal in self.s_levels[level] if literal.is_pos]
        neg_literals = [literal.symbol for literal in self.s_levels[level] if not literal.is_pos]

        # loop over all posible actions
        for action in self.all_actions:
            # create an action node out of the current action
            action_node = PgNode_a(action)
            # identify the True and False pre-conditions for each action
            b_pos = (set(action.precond_pos) - set(pos_literals) == set())
            b_neg = (set(action.precond_neg) - set(neg_literals) == set())
            # if each of the pre-conditions are met, add the resulting action as a node to the current action level
            if b_pos and b_neg:
                self.a_levels[level].add(action_node)
            # create parents for this action
            for pre_pos in action.precond_pos:
                action_node.parents.add(PgNode_s(pre_pos, True))
            for pre_neg in action.precond_neg:
                action_node.parents.add(PgNode_s(pre_neg, False))
            # create children for this action
            for eff_add in action.effect_add:
                action_node.children.add(PgNode_s(eff_add, True))
            for eff_rem in action.effect_rem:
                action_node.children.add(PgNode_s(eff_rem, False))
    ...
    def add_literal_level(self, level):
        ...
        # initialize current literal level to empty set
        self.s_levels.append(set())

        # identify the set of all positive and negative literals at the previous literal level
        pos_literals = set([literal.symbol for literal in self.s_levels[level - 1] if literal.is_pos])
        neg_literals = set([literal.symbol for literal in self.s_levels[level - 1] if not literal.is_pos])

        # map all the add and remove effects for actions at the previous action level to literals at the current level
        for action_node in self.a_levels[level - 1]:
            for effs_add in action_node.action.effect_add:
                self.s_levels[level].add(PgNode_s(effs_add, True))
            for effs_rem in action_node.action.effect_rem:
                self.s_levels[level].add(PgNode_s(effs_rem, False))

        # identify the noop actions and add their literals
        action_node_noops = [action_node for action_node in self.a_levels[level - 1] if action_node.is_persistent]
        for a_node in action_node_noops:
            for child_s_node in a_node.effnodes:
                self.s_levels[level].add(child_s_node)
                a_node.children.add(child_s_node)
                child_s_node.parents.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        b_add1_rem2 = (set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) != set()
        b_add2_rem1 = (set(node_a2.action.effect_add) & set(node_a1.action.effect_rem)) != set()
        b_inconsistent = (b_add1_rem2 or b_add2_rem1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        b1 = (set(a1_precond_pos) & set(a2_eff_rem)) != set()
        b2 = (set(a2_precond_pos) & set(a1_eff_rem)) != set()
        b3 = (set(a1_precond_neg) & set(a2_eff_add)) != set()
        b4 = (set(a2_precond_neg) & set(a1_eff_add)) != set()
        b_interference = (b1 or b2 or b3 or b4)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_parents = set([p.symbol for p in node_a1.parents])
        a2_parents = set([p.symbol for p in node_a2.parents])
        a1_parents_mutex = set([m.symbol for p in node_a1.parents for m in p.mutex])
        a2_parents_mutex = set([m.symbol for p in node_a2.parents for m in p.mutex])

        b1 = (a1_parents & a2_parents_mutex)
        b2 = (a2_parents & a1_parents_mutex)

        b_competing = (b1 or b2)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        b_negation = (node_s1 != node_s2) and (node_s1.symbol == node_s2.symbol)
        return b_negation
        # return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for p1 in node_s1.parents:
            for p2 in node_s2.parents:
                if (p1 not in p2.mutex) or (p2 not in p1.mutex):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            while (not found) and (level < max_level):
                s_level_symb = [s.symbol for s in self.s_levels[level] if s.is_pos]
                if goal in s_level_symb:
                    d_level_cost[goal] = level
                    found = True
                else:
                    level += 1
        level_sum = sum(d_level_cost.values())
        return level_sum
